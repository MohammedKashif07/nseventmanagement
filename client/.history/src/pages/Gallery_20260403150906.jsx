            ...
            <section className="pt-32 pb-12 px-6 bg-slate-50 border-b border-slate-100">
                <div className="max-w-7xl mx-auto flex flex-col gap-8">
                    ...
                    <div className="flex flex-wrap items-center gap-4">
                        ...
                        {/* Category Filter (Events/Supply) */}
                        <div className="flex bg-slate-200/50 p-1.5 rounded-2xl border border-slate-200">
                            {['all', 'events', 'supply'].map((cat) => (
                                <button
                                    key={cat}
                                    onClick={() => setCategoryFilter(cat)}
                                    className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${categoryFilter === cat ? 'bg-white text-slate-950 shadow-sm' : 'text-slate-500 hover:text-slate-900'
                                        }`}
                                >
                                    {cat}
                                </button>
